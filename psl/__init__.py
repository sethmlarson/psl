...
__version__ = "2019.9.19"

__all__ = ["PUBLIC_SUFFIX_LIST_URL", "domain_suffixes", "Suffixes", "domain_can_set_cookie"]


PUBLIC_SUFFIX_LIST_ETAG = ""
PUBLIC_SUFFIX_LIST_URL = "https://publicsuffix.org/list/public_suffix_list.dat"
PUBLIC_SUFFIX_BIN = pathlib.Path(__file__).parent / "psl.txt"
...
def domain_can_set_cookie(*, http_domain: str, cookie_domain: str) -> bool:
    ...
    """
    if "*" in http_domain or _contains_empty_labels(http_domain) or _contains_empty_labels(cookie_domain):
        return False
...
def domain_suffixes(dn: str, *, icann_only: bool=False) -> Suffixes:
    ...
    matched_rule = None
    for rule in load_public_suffix_list(icann_only=icann_only):
...
class Rule(typing.NamedTuple):
    ...
    def match(self, labels: typing.Tuple[bytes, ...]) -> bool:
        ...
        return len(labels) >= self_len and all(self.labels[i] in (b"*", labels[i]) for i in range(-self_len, 0, 1))


def load_public_suffix_list(icann_only: bool=False) -> typing.Iterable[Rule]:
    with open(PUBLIC_SUFFIX_BIN) as f:
        for line in f:
            line = line.strip()
...
def _labels_from_dn(dn: str) -> typing.Tuple[bytes, ...]:
    ...
    return tuple([typing.cast(bytes, (label.encode() if all(ord(c) <= 0x7F for c in label) else idna.encode(label, strict=True, std3_rules=True))) for label in dn.lower().split(".")])
